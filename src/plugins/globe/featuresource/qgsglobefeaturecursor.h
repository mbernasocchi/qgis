...
{
  public:
    QgsGlobeFeatureCursor( const QgsFields& fields, const QgsFeatureIterator& iterator )
        : mIterator( iterator )
        , mFields( fields )
    {
    ...
    osgEarth::Features::Feature* nextFeature() override
    {
      if ( mFeature.isValid() )
      {
        osgEarth::Features::Feature *feat = QgsGlobeFeatureUtils::featureFromQgsFeature( mFields, mFeature );
        mIterator.nextFeature( mFeature );
        return feat;
      }
      else
